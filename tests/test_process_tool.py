...
def test_run_process_echo(temp_workspace):
    """Use sys.executable -c instead of echo (shell builtin on Windows)."""
    result = run_process(
        {"program": sys.executable, "args": ["-c", "print('hello')"]},
...
def test_run_process_rejects_shell_metacharacters(temp_workspace):
    """Shell metacharacters in args are rejected regardless of program."""
    with pytest.raises(ValueError, match="Shell metacharacter"):
        run_process(
            {"program": sys.executable, "args": ["-c", "print('hello| rm -rf /')"]},
            workspace_root=str(temp_workspace),
        )
...
def test_run_process_timeout(temp_workspace):
    with pytest.raises(TimeoutError):
        run_process(
            {"program": sys.executable, "args": ["-c", "import time\ntime.sleep(10)"], "timeout": 1},
            workspace_root=str(temp_workspace),
        )
...
def test_run_process_cwd(temp_workspace):
    result = run_process(
        {"program": sys.executable, "args": ["-c", "import os\nprint(os.getcwd())"], "cwd": str(temp_workspace)},
        workspace_root=str(temp_workspace),
    )
    assert result["status"] == "SUCCESS"
...
def test_run_process_captures_stderr(temp_workspace):
    result = run_process(
        {"program": sys.executable, "args": ["-c", "import sys\nsys.stderr.write('error')"]},
        workspace_root=str(temp_workspace),
    )
...
def test_run_process_rejects_cwd_outside_workspace(temp_workspace):
    ...
    with pytest.raises(PermissionError, match="outside workspace"):
        run_process(
            {"program": sys.executable, "args": ["-c", "print('hello')"], "cwd": "/tmp"},
            workspace_root=str(temp_workspace),
        )
